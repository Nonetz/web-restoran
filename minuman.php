...
<head>
    ...
    <style>
        body {
            ...
            margin: 0;
            padding: 0;
            background-image: url('background minuman.jpg'); /* Ganti 'background.jpg' dengan URL gambar latar belakang Anda */
        ...
        .container {
            max-width: 800px;
            margin: 100px auto 50px; /* Mengatur margin atas agar konten utama tidak tertutup navbar */
            padding: 20px;
            background-color: rgba(255, 255, 255, 0.7); /* Menambahkan transparansi ke dalam container */
            border-radius: 5px;
...
<div class="navbar">
    ...
    <div class="container">
        ...
        <table>
            ...
            // Lakukan kueri SQL untuk mengambil data minuman
            $query = "SELECT * FROM minuman";
            $result = mysqli_query($koneksi, $query);

            // Periksa apakah ada data yang diambil
            if (mysqli_num_rows($result) > 0) {
                // Tampilkan data minuman dalam tabel HTML
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<tr>';
                    echo '<td>' . $row['id'] . '</td>';
                    echo '<td><img src="foto minuman/' . $row['foto'] . '" alt="' . $row['nama_minuman'] . '" class="food-image"></td>';
                    echo '<td>' . $row['nama_minuman'] . '</td>';
                    echo '<td>Rp ' . number_format($row['harga'], 0, ',', '.') . '</td>';
                    echo '<td>
                             <a href="feedback.php?id=' . $row['id'] . '"><button class="options-btn">Berikan Feedback</button></a>
                            <a href="pesan.minuman.php?id=' . $row['id'] . '"><button class="options-btn">Pesan</button></a>
                          </td>';
                    echo '</tr>';
                }
            } else {
